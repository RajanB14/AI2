    ...
    with torch.no_grad():
        while win32api.GetAsyncKeyState(ord(aaQuitKey)) == 0:
            ...
            if not targets_sorted.empty:
                ...
                # Check if Caps Lock is toggled on and if the left mouse button is being held down
                if win32api.GetKeyState(0x14) & 1:  # If the least significant bit is set, Caps Lock is toggled on
                    if win32api.GetKeyState(0x01) < 0:  # Negative value indicates the button is down
                        # Move the mouse
                        win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, int(smoothed_mouseMove[0] * aaMovementAmp), int(smoothed_mouseMove[1] * aaMovementAmp), 0, 0)
