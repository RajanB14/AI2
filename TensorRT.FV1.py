...
def main():
    ...
    # Initialize a variable to keep track of the locked target
    locked_target = None
    lock_threshold = 10  # Threshold for locking onto a new target due to significant movement
    ...
    # Function to check if the mouse has moved significantly
    def has_mouse_moved_significantly(threshold=10):
        current_mouse_pos = win32api.GetCursorPos()
        if abs(current_mouse_pos[0] - previous_mouse_move[0]) > threshold or \
            abs(current_mouse_pos[1] - previous_mouse_move[1]) > threshold:  # Compare current mouse position to the last saved one
             return True
        return False
    ...
    with torch.no_grad():
        while win32api.GetAsyncKeyState(ord(aaQuitKey)) == 0:
            ...
            # Inverse area to simulate 'distance', smaller area represents a larger 'dist'
            targets['dist'] = 1 / targets['area']
            # Now sort by 'dist' to have targets with smaller 'dist' first (closer to the player)
            targets_sorted = targets.sort_values(by="dist", ascending=True)
            ...
            if not targets_sorted.empty:
                ...
                if locked_target is not None and not has_mouse_moved_significantly():
                    # Calculate the distance between the current target and the locked target
                    dist_to_locked = np.linalg.norm(np.array((selected_target['xMid'], selected_target['yMid'])) - 
                                                    np.array((locked_target['xMid'], locked_target['yMid'])))
                    # If the current target hasn't moved beyond the threshold, keep the locked target
                    if dist_to_locked < lock_threshold:
                        selected_target = locked_target
                    else:
                        # Target moved significantly, update lock
                        locked_target = selected_target
                else:
                    # No locked target or mouse moved, lock to closest target
                    locked_target = selected_target
